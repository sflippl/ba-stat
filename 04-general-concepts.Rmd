...
`fx_output` is stable and the next step will be to refine and extend the output forms. In particular, the next version will provide an output form "report" which creates a dynamical report of all variables.
...
metaframe(fx_diamonds) %>% 
  select(name, fxInfo_name:fxInfo_unit)
```
...
fx_diamonds %>% 
  ...
  kable(
    booktabs = TRUE, 
    caption = "Metaframe of diamonds with modified parameters (Extract)"
  )
```
...
```{r title, fig.cap = "(ref:title)", out.width='50%', fig.align='center'}
